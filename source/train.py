...
def run(weights="models",  # pylint: disable=too-many-arguments, too-many-locals
    ...
    with open(save_dir / 'opt.yaml', 'w', encoding="UTF-8") as file:
        yaml.safe_dump(OptArgs.instance().args, file,
                       sort_keys=False)  # pylint: disable=no-member
    with open(save_dir / 'hyp.yaml', 'w', encoding="UTF-8") as file:
        yaml.safe_dump(
            yml_hyp, file, sort_keys=False)  # pylint: disable=no-member
...
if __name__ == "__main__":
    ...
    if is_master_process(RANK):  # Master Process 0 or -1
        set_logging(PREFIX)
        check_requirements()
    time = timeit.timeit(lambda: run(**opt_args),
                         number=1)  # pylint: disable=unnecessary-lambda
