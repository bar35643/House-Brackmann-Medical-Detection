...
Die Eingabe $w$ muss nicht zwangsläufig sich auf den Wertebereich $\{0, 1 \}\ast$ beziehen. Aufgabegebiete solcher Maschinen beinhalten Steuerungsaufgaben, Mustererkennung in Daten, Netzwerkprotokolle, Sprachverarbeitung, Klassifizierungen und anderweitige Anwendunsmöglichkeiten. Es gibt zwei Hauptkategorien von Automatenklassen \acp{dea} und \acp{nea}, die unterschieden werden.
...
Ein \ac{nea} ist formal durch ein 5-Tupel $M = (Q, \Sigma, \delta, E, F)$ definiert. Der unterschied zwischen \ac{dea} und \ac{nea}, ist, dass mehere Startzustände vorhanden sein können und die Übergangsfunktionen nicht alle Formal definiert sein müssen. Das 5-Tupel für den \ac{nea} enthält folgende Komponenten:
...
\node []               at (xaxis) {Berechnete Klasse aus einem Neuronalen Netz ($p$)};
\end{tikzpicture}
...
Im folgenden Kapitel soll der momentane  Stand der Technik für die zugrunge liegende Aufgabenstellung erläutert werden. Dabei werden 2 verschiedenste vorangehensweisen kurz erläutert. Der Abschnitt \ref{segmentation} veranschaulicht dabei die Graduierung der Fazialisparese duch die Anwendung von Segmentierung zur Extraktion der wichtigsten Gesichtseigenschaften. Eine andere Herangehensweise ist mit Videoframes vergleichsbasiert anhand einer aufgenommenen Referensstellung den \ac{hb_grade} festzustellen (Kapitel \ref{compare}).
...
%\caption[Darstellung der Flowchart vom Paper {Automatic Facial Paralysis Evaluation Augmented by a Cascaded Encoder Network Structure]{Darstellung der Flowchart vom Paper Automatic Facial Paralysis Evaluation Augmented by a Cascaded Encoder Network Structure. \cite{detection_fp2}}\label{cap:paper_1}
